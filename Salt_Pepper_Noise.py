# This code finds the accuracy of extraction after adding Salt and Pepper noise to the watermarked image
...
def add_salt_and_pepper_noise(img, noise_percentage):
    noisy_img = np.copy(img)
    if len(noisy_img.shape) == 2:  # Grayscale image
        rw, cl = noisy_img.shape
        num_pixels = int((noise_percentage / 100) * (rw * cl))
        for _ in range(num_pixels):
            # Salt noise
            y_crd = random.randint(0, rw-1)
            x_crd = random.randint(0, cl-1)
            noisy_img[y_crd, x_crd] = 255  # Set the pixel to white (255)

            # Pepper noise
            y_crd = random.randint(0, rw-1)
            x_crd = random.randint(0, cl-1)
            noisy_img[y_crd, x_crd] = 0  # Set the pixel to black (0)
    elif len(noisy_img.shape) == 3:  # Color image
        rw, cl, ch = noisy_img.shape
        num_pixels = int((noise_percentage / 100) * (rw * cl))
        for _ in range(num_pixels):
            # Salt noise
            y_crd = random.randint(0, rw-1)
            x_crd = random.randint(0, cl-1)
            noisy_img[y_crd, x_crd, :] = 255  # Set all channels to white (255)

            # Pepper noise
            y_crd = random.randint(0, rw-1)
            x_crd = random.randint(0, cl-1)
            noisy_img[y_crd, x_crd, :] = 0  # Set all channels to black (0)
    return noisy_img
...
for i, url in enumerate(urls, start=1):
    ...
    # watermarked_image_noisy = add_salt_and_pepper_noise(watermarked_image)
  ...
  # Apply 5% noise
    watermark_sp = add_salt_and_pepper_noise(watermarked_image, noise_percentage=5)
    ...
    cv2.imwrite(f"watermarked_s&p1_{host_path}", cv2.resize(watermark_sp, (1024, 1024)))
    
    coeffs_w = pywt.dwt2(watermark_sp, wavelet)
    ...
    for k in range(row_hh):
        for l in range(cols_hh - k, cols_hh):
            if l >= 0:  # Check if the index is valid
                HH2_w[k, l] = 0
    cv2.imshow('zeros',cv2.resize(watermark_dct,(418,418)))

    # Extract the watermark from the HH subband
    extr = cv2.idct(HH2_w)
    extr = cv2.resize(extr, (256, 256))
    cv2.imwrite(f"extracted_watermark_s&p1_{host_path}", extr)
    image_path = f"extracted_watermark_s&p1_{host_path}"

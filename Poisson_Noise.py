# This code finds the accuracy of extraction after adding Poisson noise to the watermarked image
...
def add_scaled_poisson_noise(image, scale_factor):
    if image is None:
        print("Error: Image not loaded.")
        return None

    lam = image.astype(float) / 255.0 * scale_factor  # Scale lambda by a factor
    lam = np.clip(lam, 1e-6, None)  # Clip values to avoid negative or zero lam
    noise = np.random.poisson(lam).astype(image.dtype)  # Generate Poisson noise
    noisy_image = cv2.add(image, noise)  # Add Poisson noise to the image
    return noisy_image
...
for i, url in enumerate(urls, start=1):
    ...
    scale_factor = 10 # Adjust this factor to control the intensity of the noise
    noisy_image = add_scaled_poisson_noise(watermarked_image, scale_factor)
    ...
    cv2.imwrite(f"watermarked_poisson_10_{host_path}", cv2.resize(noisy_image, (1024, 1024)))
    ...
    cv2.imwrite(f"extracted_watermark_poisson_10_{host_path}", extr)
    image_path = f"extracted_watermark_poisson_10_{host_path}"
